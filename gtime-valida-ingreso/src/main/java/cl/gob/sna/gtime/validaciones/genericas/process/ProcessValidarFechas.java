...
public class ProcessValidarFechas implements Processor {
	...
	final Logger LOG = Logger.getLogger(ProcessValidarFechas.class);
	...
	public void process(Exchange exchange) throws Exception {
		LOG.info("inicio validacion de fechas");
		...
		if (fechasRepetidas == null) {
			...
			for (Fecha fecha : docResponse.getGtime().getFechas().getFecha()) {
				...
				if(fem==null) { 
 					fem = validacionFechasService.isFem(fecha.getNombre()) ?
 								validacionFechasService.stringToDate(fecha.getNombre(), fecha.getValor())
 												: null;
 				}				
				ordinality++;
			}
