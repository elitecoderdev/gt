...
public class ProcessValidacionTotalItem implements Processor {

	Logger log = Logger.getLogger(ProcessValidacionTotalItem.class);
	public void process(Exchange exchange) throws Exception {	
		DocumentoResponse docResponse = exchange.getIn().getHeader("docResponse",DocumentoResponse.class);
		Validacion val = null;
		if (StringValidation.isNull(docResponse.getGtime().getTotalItem())) {
			val = new Validacion(TipoValidacion.TOTAL_ITEM_NULL, false, new Date());
		} else {
			val = new Validacion(TipoValidacion.TOTAL_BULTOS, 
								 StringValidation.isNumeric(docResponse.getGtime().getTotalItem()) 
								 	&& new BigDecimal(docResponse.getGtime().getTotalItem()).compareTo(BigDecimal.ZERO) > 0,
								 new Date());
 		}
